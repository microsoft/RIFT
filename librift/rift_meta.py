...
RE_RUSTLIB_PATTERN = r"(?:^|[\\/])([^\\/]{1,50}-\d+\.\d+\.\d+(?:-[^\\/]{1,20})?)[\\/]src[\\/].{1,100}\.rs"
...
class RiftMeta:
    ...
    def _extract_from_strings(self, strings: List[str], filetype=None, arch=None) -> RustMetadata:
        ...
        for s in strings:
            ...
            if re.search(r"(github|\.cargo|\.rustup|rustc|library|crates\.io|rust[\\|\/]deps|[\\|\/]deps[\\|\/])", s) is not None and (match := self.__rustlib_re.search(s)):
                crate = match.group(1)
